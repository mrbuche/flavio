...
fn first_order_tensor_rank_0() {
    ...
    let solution: TensorRank0List<LENGTH> =
        ode45(|t: &f64, _: &TensorRank0| t.cos(), &evaluation_times, 0.0).expect("the unexpected");
    evaluation_times
        .iter()
        .zip(solution.iter())
        .for_each(|(t, y)| {
...
fn second_order_tensor_rank_0() {
    let evaluation_times = zero_to_tau::<LENGTH>();
    let solution: TensorRank1List<2, 1, LENGTH> = ode45(
...
fn third_order_tensor_rank_0() {
    let evaluation_times = zero_to_tau::<LENGTH>();
    let solution: TensorRank1List<3, 1, LENGTH> = ode45(
...
fn fourth_order_tensor_rank_0() {
    let evaluation_times = zero_to_tau::<LENGTH>();
    let solution: TensorRank1List<4, 1, LENGTH> = ode45(
