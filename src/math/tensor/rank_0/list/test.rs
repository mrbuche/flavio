...
fn from_iter()
{
    ...
    tensor_rank_0_list.iter()
    .zip(into_iterator)
    .for_each(|(tensor_rank_0_list_i, value_i)|
        assert_eq!(tensor_rank_0_list_i, &value_i)
    );
}
...
fn iter()
{
    get_tensor_rank_0_list().iter()
    .zip(get_array().iter())
    .for_each(|(tensor_rank_0_list_i, array_i)|
        assert_eq!(tensor_rank_0_list_i, array_i)
    );
}
...
fn iter_mut()
{
    get_tensor_rank_0_list().iter_mut()
...
fn zero()
{
    TensorRank0List::<8>::zero().iter()
    .for_each(|tensor_rank_0_list_i|
        assert_eq!(tensor_rank_0_list_i, &0.0)
    );
}
