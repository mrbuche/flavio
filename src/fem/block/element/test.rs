...
macro_rules! setup_for_surface_or_localization_elements
{
    ($element: ident) =>
    {
        ...
        {
            crate::mechanics::test::get_rotation_reference_configuration().transpose().into()
        }
        crate::fem::block::element::test::setup_for_element_tests_any_element!($element);
    }
}
                ...
                .for_each(|(b, nodal_stiffness_ab)|
                    ...
                    .for_each(|((i, nodal_stiffness_ab_i), nodal_stiffness_ab_j)|
                        ...
                        .for_each(|((j, nodal_stiffness_ab_ij), nodal_stiffness_ab_ji)|
                            ...
                            {
                                assert_eq_within_tols(
                                    nodal_stiffness_ab_ij, &nodal_stiffness_ab_ji
                                )
                            }
                            else if i == j
                            {
        ...
        fn get_nodal_forces(is_deformed: bool, is_rotated: bool, is_xtra: bool) -> NodalForces<N>
        {
            if is_xtra
            {
                if is_rotated
                {
                    ...
                    {
                        get_element().calculate_nodal_forces(
                            &get_reference_coordinates_transformed().into(), &NodalVelocities::zero()
                        )
                    }
                }
                else
                {
                    ...
                    {
                        get_element().calculate_nodal_forces(
                            &get_reference_coordinates().into(), &NodalVelocities::zero()
                        )
                    }
                }
            }
                ...
                fn minimized()
                {
                    ...
                    (0..N).for_each(|node|
                        (0..3).for_each(|i|{
                            ...
                            perturbed =
                                element.calculate_viscous_dissipation(
                                    &get_reference_coordinates().into(), &perturbed_velocities
                                );
                            assert!(
                                perturbed > minimum ||
                                check_eq_within_tols(&perturbed, &minimum)
