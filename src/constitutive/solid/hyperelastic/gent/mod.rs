...
impl<'a> Elastic<'a> for Gent<'a>
{
    ...
    fn calculate_cauchy_stress(&self, deformation_gradient: &DeformationGradient) -> CauchyStress
    {
        ...
        (deviatoric_isochoric_left_cauchy_green_deformation*self.get_shear_modulus()*self.get_extensibility()/jacobian)/denominator + IDENTITY*self.get_bulk_modulus()*0.5*(jacobian - 1.0/jacobian)
    }
...
impl<'a> Hyperelastic<'a> for Gent<'a>
{
    ...
    fn calculate_helmholtz_free_energy_density(&self, deformation_gradient: &DeformationGradient) -> Result<Scalar, ConstitutiveError>
    {
        ...
        if jacobian > 0.0 {
            ...
            if factor >= 1.0 {
                Err(ConstitutiveError::Custom("Maximum extensibility reached.".to_string(), deformation_gradient.copy(), format!("{:?}", &self)))
            } else {
