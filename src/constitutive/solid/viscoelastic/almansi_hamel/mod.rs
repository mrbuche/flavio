...
impl<'a> Viscoelastic<'a> for AlmansiHamel<'a>
{
    ...
    fn calculate_cauchy_stress(&self, deformation_gradient: &DeformationGradient, deformation_gradient_rate: &DeformationGradientRate) -> CauchyStress
    {
        ...
        deviatoric_strain*(2.0*self.get_shear_modulus()) + deviatoric_strain_rate*(2.0*self.get_shear_viscosity()) + CauchyStress::identity()*(self.get_bulk_modulus()*strain_trace + self.get_bulk_viscosity()*strain_rate_trace)
    }
    ...
    fn calculate_cauchy_rate_tangent_stiffness(&self, deformation_gradient: &DeformationGradient, _: &DeformationGradientRate) -> CauchyRateTangentStiffness
    {
        ...
        let scaled_deformation_gradient_inverse_transpose = &deformation_gradient_inverse_transpose * self.get_shear_viscosity();
