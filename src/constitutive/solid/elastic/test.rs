...
macro_rules! calculate_first_piola_kirchoff_tangent_stiffness_from_deformation_gradient
{
    ($constitutive_model_constructed: expr, $deformation_gradient: expr) =>
    {
        $constitutive_model_constructed.calculate_first_piola_kirchoff_tangent_stiffness($deformation_gradient)
    }
}
...
macro_rules! calculate_second_piola_kirchoff_stress_from_deformation_gradient
{
    ($constitutive_model_constructed: expr, $deformation_gradient: expr) =>
    {
        $constitutive_model_constructed.calculate_second_piola_kirchoff_stress($deformation_gradient)
    }
}
...
macro_rules! calculate_second_piola_kirchoff_tangent_stiffness_from_deformation_gradient
{
    ($constitutive_model_constructed: expr, $deformation_gradient: expr) =>
    {
        $constitutive_model_constructed.calculate_second_piola_kirchoff_tangent_stiffness($deformation_gradient)
    }
}
...
mod temporary {
        ...
        fn calculate_second_piola_kirchoff_tangent_stiffness_from_finite_difference_of_second_piola_kirchoff_stress(is_deformed: bool) -> SecondPiolaKirchoffTangentStiffness
        {
            ...
            for k in 0..3
            {
                for l in 0..3
                {
                    ...
                    deformation_gradient_plus[k][l] += 0.5*EPSILON;
                    let second_piola_kirchoff_stress_plus = calculate_second_piola_kirchoff_stress_from_deformation_gradient!(&model, &deformation_gradient_plus);
                    let mut deformation_gradient_minus = 
                        if is_deformed
                        {
                            get_deformation_gradient()
                    ...
                    deformation_gradient_minus[k][l] -= 0.5*EPSILON;
                    let second_piola_kirchoff_stress_minus = calculate_second_piola_kirchoff_stress_from_deformation_gradient!(&model, &deformation_gradient_minus);
                    for i in 0..3
                    {
                        for j in 0..3
                        {
                            second_piola_kirchoff_tangent_stiffness[i][j][k][l] = (second_piola_kirchoff_stress_plus[i][j] - second_piola_kirchoff_stress_minus[i][j])/EPSILON;
                        }
                    }
                }
            }
        ...
        mod first_piola_kirchoff_stress
        {
            ...
            //     fn finite_difference()
            //     {
            //         $elastic_constitutive_model_constructed.calculate_first_piola_kirchoff_tangent_stiffness(&DeformationGradient::identity()).iter()
            ...
            //     fn zero()
            //     {
            //         $elastic_constitutive_model_constructed.calculate_first_piola_kirchoff_stress(&DeformationGradient::identity()).iter()
...
macro_rules! test_elastic_constitutive_model
{
    ($elastic_constitutive_model: ident, $elastic_constitutive_model_parameters: expr, $elastic_constitutive_model_constructed: expr) =>
    {
        ...
        fn get_bulk_modulus()
        {
            assert_eq!(&$elastic_constitutive_model_parameters[0], get_elastic_constitutive_model().get_bulk_modulus())
        }
        #[test]
        fn get_shear_modulus()
        {
            assert_eq!(get_elastic_constitutive_model().get_shear_modulus(), &$elastic_constitutive_model_parameters[1])
        }
        #[test]
        fn bulk_modulus()
        {
            let model = get_elastic_constitutive_model();
            let deformation_gradient = DeformationGradient::identity() * (1.0 + EPSILON).powf(1.0/3.0);
...
pub(crate) use test_elastic_constitutive_model;
macro_rules! test_elastic_constitutive_model_constructed
{
    ($elastic_constitutive_model_constructed: expr) =>
    {
        use crate::
        {
            EPSILON,
        ...
        fn calculate_cauchy_tangent_stiffness_from_finite_difference_of_cauchy_stress(is_deformed: bool) -> CauchyTangentStiffness
        {
            let model = $elastic_constitutive_model_constructed;
            let mut cauchy_tangent_stiffness = CauchyTangentStiffness::zero();
            for k in 0..3
            {
                for l in 0..3
        ...
        fn calculate_second_piola_kirchoff_tangent_stiffness_from_finite_difference_of_second_piola_kirchoff_stress(is_deformed: bool) -> SecondPiolaKirchoffTangentStiffness
        {
            ...
            for k in 0..3
            {
                for l in 0..3
                {
                    ...
                    deformation_gradient_plus[k][l] += 0.5*EPSILON;
                    let second_piola_kirchoff_stress_plus = model.calculate_second_piola_kirchoff_stress(&deformation_gradient_plus);
                    let mut deformation_gradient_minus = 
                        if is_deformed
                        {
                            get_deformation_gradient()
                    ...
                    deformation_gradient_minus[k][l] -= 0.5*EPSILON;
                    let second_piola_kirchoff_stress_minus = model.calculate_second_piola_kirchoff_stress(&deformation_gradient_minus);
                    for i in 0..3
                    {
                        for j in 0..3
                        {
        ...
        mod cauchy_stress
        {
            ...
            mod deformed
            {
                ...
                fn finite_difference()
                {
                    $elastic_constitutive_model_constructed.calculate_cauchy_tangent_stiffness(&get_deformation_gradient()).iter()
            ...
            mod undeformed
            {
                ...
                fn finite_difference()
                {
                    $elastic_constitutive_model_constructed.calculate_cauchy_tangent_stiffness(&DeformationGradient::identity()).iter()
                ...
                fn zero()
                {
                    $elastic_constitutive_model_constructed.calculate_cauchy_stress(&DeformationGradient::identity()).iter()
        ...
        mod first_piola_kirchoff_stress
        {
            ...
            mod deformed
            {
                ...
                fn finite_difference()
                {
                    $elastic_constitutive_model_constructed.calculate_first_piola_kirchoff_tangent_stiffness(&get_deformation_gradient()).iter()
                    .zip(calculate_first_piola_kirchoff_tangent_stiffness_from_finite_difference_of_first_piola_kirchoff_stress(true).iter())
                    .for_each(|(first_piola_kirchoff_tangent_stiffness_i, fd_first_piola_kirchoff_tangent_stiffness_i)|
                        first_piola_kirchoff_tangent_stiffness_i.iter()
                        .zip(fd_first_piola_kirchoff_tangent_stiffness_i.iter())
        ...
        mod second_piola_kirchoff_stress
        {
            ...
            mod deformed
            {
                ...
                fn finite_difference()
                {
                    $elastic_constitutive_model_constructed.calculate_second_piola_kirchoff_tangent_stiffness(&get_deformation_gradient()).iter()
                ...
                {
                    let model = $elastic_constitutive_model_constructed;
                    model.calculate_second_piola_kirchoff_stress(&get_deformation_gradient()).iter()
                    .zip((
                        get_rotation_reference_configuration().transpose() *
                        model.calculate_second_piola_kirchoff_stress(&get_deformation_gradient_rotated()) *
                        get_rotation_reference_configuration()
                    ).iter())
            ...
            mod undeformed
            {
                ...
                fn finite_difference()
                {
                    $elastic_constitutive_model_constructed.calculate_second_piola_kirchoff_tangent_stiffness(&DeformationGradient::identity()).iter()
                ...
                fn zero()
                {
                    $elastic_constitutive_model_constructed.calculate_second_piola_kirchoff_stress(&DeformationGradient::identity()).iter()
                    .for_each(|second_piola_kirchoff_stress_i|
                        second_piola_kirchoff_stress_i.iter()
        ...
        mod second_piola_kirchoff_tangent_stiffness
        {
            ...
            mod deformed
            {
                ...
                {
                    let model = $elastic_constitutive_model_constructed;
                    model.calculate_second_piola_kirchoff_tangent_stiffness(&get_deformation_gradient()).iter()
                    .zip((
                        model.calculate_second_piola_kirchoff_tangent_stiffness(&get_deformation_gradient_rotated())
                        .contract_all_indices_with_first_indices_of(
                            &get_rotation_reference_configuration(),
                            &get_rotation_reference_configuration(),
                            &get_rotation_current_configuration(),
...
pub(crate) use test_elastic_constitutive_model_constructed;
macro_rules! test_elastic_only_constitutive_model_constructed
{
    ($elastic_constitutive_model_constructed: expr) =>
    {
        mod elastic_only
...
pub(crate) use test_elastic_only_constitutive_model_constructed;