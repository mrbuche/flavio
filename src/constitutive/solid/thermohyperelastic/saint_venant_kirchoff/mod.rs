...
impl<'a> Thermoelastic<'a> for SaintVenantKirchoff<'a>
{
    ...
    fn calculate_second_piola_kirchoff_stress(&self, deformation_gradient: &DeformationGradient, temperature: &Scalar) -> SecondPiolaKirchoffStress
    {
        ...
        deviatoric_strain*(2.0*self.get_shear_modulus()) + IDENTITY_00*(self.get_bulk_modulus()*(strain_trace - 3.0*self.get_coefficient_of_thermal_expansion()*(temperature - self.get_reference_temperature())))
    }
    ...
    /// ```
    fn calculate_second_piola_kirchoff_tangent_stiffness(&self, deformation_gradient: &DeformationGradient, _: &Scalar) -> SecondPiolaKirchoffTangentStiffness
    {
...
impl<'a> Thermohyperelastic<'a> for SaintVenantKirchoff<'a>
{
    ...
    fn calculate_helmholtz_free_energy_density(&self, deformation_gradient: &DeformationGradient, temperature: &Scalar) -> Result<Scalar, ConstitutiveError>
    {
        ...
        if jacobian > 0.0 {
            ...
            Err(ConstitutiveError::InvalidJacobianThermoelastic(jacobian, deformation_gradient.copy(), *temperature, format!("{:?}", &self)))
        }
    }
}
