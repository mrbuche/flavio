...
fn thermal_conductivity()
{
    ...
    model.calculate_heat_flux(
        &get_temperature_gradient()
    ).iter().zip((
        get_temperature_gradient() / -model.get_thermal_conductivity()
    ).iter()).for_each(|(heat_flux_i, entry_i)|
        assert_eq!(heat_flux_i, entry_i)
    )
}
...
fn zero()
{
    get_constitutive_model().calculate_heat_flux(
        &TemperatureGradient::new([0.0, 0.0, 0.0])
    ).iter().for_each(|heat_flux_i|
        assert_eq!(heat_flux_i, &0.0)
    )
}